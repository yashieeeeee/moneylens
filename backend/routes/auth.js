...
router.post("/signup", async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign({ userId: String(result.lastInsertRowid), email }, JWT_SECRET, { expiresIn: "3650d" });
...
router.post("/login", async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign({ userId: String(user.id), email: user.email }, JWT_SECRET, { expiresIn: "3650d" });
